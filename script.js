...
function renderQuestion(){
  ...
  renderMathIn(document.querySelector('.quiz-body'));
}

// Renders LaTeX math notation (wrapped in $...$ or $$...$$) into properly
// formatted equations, using KaTeX. Safe to call even if a question has
// no math in it at all — it just does nothing in that case. Also safe if
// the KaTeX CDN failed to load (e.g. no internet) — falls back to showing
// the raw $...$ text rather than breaking the page.
...
function selectOption(i){
  ...
  renderMathIn(document.getElementById('explainBox'));
...
function submitExam(){
  ...
  quizState.questions.forEach((q, idx) => {
    ...
    const selectedPosition = quizState.userAnswers[idx];
    const isCorrect = selectedPosition === shuffle.correctPosition;
    if(isCorrect) score++;
    reviewData.push({
      stem: q.stem,
      options: shuffle.options,
      correctPosition: shuffle.correctPosition,
      selectedPosition: selectedPosition, // null if left unanswered
      explain: q.explain,
      isCorrect: isCorrect
    });
  });
  quizState.score = score;
  sessionStorage.setItem('enzymed_exam_review', JSON.stringify(reviewData));
  finishQuiz();
}
...
function finishQuiz(){
  ...
  // Quiz mode never generates review data — clear out any stale review
  // from a previous exam attempt so results.html doesn't show a leftover
  // "Review answers" button that doesn't belong to this attempt.
  if(quizState.mode !== 'exam'){
    sessionStorage.removeItem('enzymed_exam_review');
  }
...
function renderResultsPage(){
  ...
  if(reviewBtn){
    const hasReviewData = sessionStorage.getItem('enzymed_exam_review');
    if(hasReviewData){
      reviewBtn.style.display = 'block';
      reviewBtn.onclick = () => window.location.href = 'review.html';
    } else {
...
function getUserName(){
  try {
    return localStorage.getItem('enzymed_user_name') || '';
  } catch(e) {
    return '';
  }
}
function saveUserName(name){
  try {
    localStorage.setItem('enzymed_user_name', name.trim());
  } catch(e) {
...
function clearHistory(){
  const confirmed = window.confirm('Clear all saved quiz history? This can\'t be undone.');
  if(!confirmed) return;
  try {
    localStorage.removeItem('enzymed_quiz_history');
  } catch(e) {
    console.error('Could not clear history:', e);
  }
  renderHistoryPage(); // re-render immediately to show the empty state
}
...
function renderHistoryPage(){
  ...
  // Name capture — purely local, a label, not a real account.
  const nameCapture = document.getElementById('nameCapture');
  const subtitle = document.getElementById('historySubtitle');
  const existingName = getUserName();

  if(existingName){
    subtitle.textContent = `${existingName}'s quiz history`;
    nameCapture.style.display = 'none';
  } else {
    subtitle.textContent = 'Your quiz history';
    nameCapture.style.display = 'block';
  }
  ...
  if(nameSaveBtn){
    nameSaveBtn.onclick = () => {
      ...
      saveUserName(val);
      subtitle.textContent = `${val}'s quiz history`;
      nameCapture.style.display = 'none';
    };
  }
  // Tapping the subtitle lets you edit the name later.
  subtitle.onclick = () => {
    nameCapture.style.display = 'block';
  ...
  history.forEach(entry => {
    ...
    card.innerHTML = `
      ...
      <div>
        ...
        <div class="course-meta">${entry.score} / ${entry.total} (${pct}%) · ${dateStr} at ${timeStr}</div>
      </div>
    `;
...
/* ============================================
   REVIEW ANSWERS PAGE (exam mode only)
   ============================================ */
function renderReviewPage(){
  const list = document.getElementById('reviewList');
  if(!list) return; // not on review.html

  let reviewData = [];
  try {
    reviewData = JSON.parse(sessionStorage.getItem('enzymed_exam_review') || '[]');
  } catch(e) {
    console.error('Could not read exam review data:', e);
  }

  if(reviewData.length === 0){
    list.innerHTML = '<div class="topic-overview" style="margin-top:0;">No recent exam to review — this is only available right after finishing an Exam Mode attempt.</div>';
    return;
  }
  ...
  reviewData.forEach((q, idx) => {
    ...
    q.options.forEach((opt, i) => {
      ...
      if(i === q.correctPosition) cls += ' review-correct';
      else if(i === q.selectedPosition) cls += ' review-incorrect';
      optionsHtml += `<div class="${cls}"><span class="review-opt-letter">${letters[i]}</span>${opt}</div>`;
    });

    const statusBadge = q.isCorrect
      ? '<span class="review-badge review-badge-correct">Correct</span>'
      : (q.selectedPosition === null
          ? '<span class="review-badge review-badge-skipped">Skipped</span>'
          : '<span class="review-badge review-badge-wrong">Wrong</span>');
    ...
    card.innerHTML = `
      ...
      <div class="explain-box" style="display:block;"><b>Why:</b> ${q.explain}</div>
    `;
    list.appendChild(card);
  });
